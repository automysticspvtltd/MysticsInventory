...
type Allocation = { salesOrderId: number; amount: string };
...
export function RecordPaymentDialog({
  ...
  const { data: openOrders, isLoading: ordersLoading } = useListSalesOrders(
    { customerId },
    {
      query: {
        enabled: open && !!customerId,
        queryKey: getListSalesOrdersQueryKey({ customerId }),
      },
    },
  );
    ...
    () =>
      (openOrders ?? []).filter(
        (o) =>
          ...
          ["confirmed", "shipped", "delivered", "invoiced"].includes(o.status),
      ),
    [openOrders],
  );
      ...
      setAllocations([
        {
          ...
          amount: presetSalesOrderBalance.toFixed(2),
        },
      ]);
    } else {
      setAllocations([]);
    }
  }, [open, presetSalesOrderId, presetSalesOrderBalance]);
  ...
  const toggleAllocation = (orderId: number, balance: number) => {
    setAllocations((prev) => {
      const exists = prev.find((a) => a.salesOrderId === orderId);
      if (exists) return prev.filter((a) => a.salesOrderId !== orderId);
      return [...prev, { salesOrderId: orderId, amount: balance.toFixed(2) }];
    });
  };
  ...
  const updateAllocationAmount = (orderId: number, value: string) => {
    setAllocations((prev) =>
      prev.map((a) =>
        a.salesOrderId === orderId ? { ...a, amount: value } : a,
      ),
    );
  };
  ...
  const createMutation = useCreateCustomerPayment({
    mutation: {
      onSuccess: () => {
        ...
        if (presetSalesOrderId) {
          queryClient.invalidateQueries({
            queryKey: getGetSalesOrderQueryKey(presetSalesOrderId),
          });
        }
